...
// .wrangler/tmp/bundle-8D0oBh/checked-fetch.js
...
// .wrangler/tmp/bundle-8D0oBh/strip-cf-connecting-ip-header.js
...
async function handleUpdateUsername(request, env) {
  try {
    ...
    const { walletAddress, newUsername, signature, message, timestamp, nonce } = data;
...
async function verifySession(db, sessionToken, walletAddress) {
  try {
    ...
    ).bind(sessionToken, walletAddress).first();
    return session;
  } catch (error) {
...
function verifyEthereumSignature(message, signature, expectedAddress) {
  try {
    const recoveredAddress = ethers_exports.verifyMessage(message, signature);
...
// .wrangler/tmp/bundle-8D0oBh/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-8D0oBh/middleware-loader.entry.ts
