...
// .wrangler/tmp/bundle-rU57tK/checked-fetch.js
...
function assertArgumentCount(count, expectedCount, message2) {
  ...
  assert(count >= expectedCount, "missing argument" + message2, "MISSING_ARGUMENT", {
    count,
    expectedCount
  });
...
var TypedDataEncoder = class _TypedDataEncoder {
   ...
   */
  getEncoder(type) {
    let encoder2 = this.#encoderCache.get(type);
    if (!encoder2) {
      encoder2 = this.#getEncoder(type);
      this.#encoderCache.set(type, encoder2);
    }
    return encoder2;
  }
  #getEncoder(type) {
  ...
  static hash(domain, types2, value) {
    return keccak256(_TypedDataEncoder.encode(domain, types2, value));
  }
...
function lex(text) {
  const tokens = [];
  const throwError2 = /* @__PURE__ */ __name((message2) => {
...
var BuiltinErrors = {
  "0x08c379a0": {
    ...
    name: "Error",
    inputs: ["string"],
    reason: /* @__PURE__ */ __name((message2) => {
...
// src/index.js
var JWT_SECRET = "your-super-secret-jwt-key-change-this-in-production";
...
async function handleUpdateUsername(request, env) {
  try {
    ...
    const { walletAddress, newUsername, signature, message: message2, timestamp, nonce } = data;
    if (!walletAddress || !newUsername || !signature || !message2) {
    ...
    if (!existingUser) {
      ...
      `
      ).bind(walletAddress, newUsername).run();
    } else {
      await env.DB.prepare(
        `
...
__name(handleUpdateUsername, "handleUpdateUsername");
function verifyEthereumSignature(message2, signature, expectedAddress) {
  try {
    const recoveredAddress = ethers_exports.verifyMessage(message2, signature);
...
__name(verifyEthereumSignature, "verifyEthereumSignature");
async function generateJWTToken(walletAddress, username = null) {
  try {
    ...
    const token = await new SignJWT({
      ...
      username,
      type: "auth"
    }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(JWT_EXPIRES_IN).setIssuer("web3-workers").setAudience("web3-course").sign(secret);
    return token;
  } catch (error) {
...
async function handleAuth(request, env) {
  try {
    const data = await request.json();
    const { walletAddress, signature, message: message2, timestamp, nonce } = data;
    if (!walletAddress || !signature || !message2) {
      return createCorsResponse(
        {
          success: false,
          error: "\u7F3A\u5C11\u5FC5\u8981\u53C2\u6570"
    ...
    await env.DB.prepare(
      ...
      VALUES (?, ?, ?)
    `
    ).bind(walletAddress, sessionToken, expiresAt.toISOString()).run();
...
// .wrangler/tmp/bundle-rU57tK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rU57tK/middleware-loader.entry.ts
